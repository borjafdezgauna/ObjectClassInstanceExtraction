...
class BaseTrainer:
    ...
    def _save_model(self, save_path: str, model: PreTrainedModel, tokenizer: PreTrainedTokenizer,
        ...
        if isinstance(model, (DataParallel, DistributedDataParallel)):
            ...
            # This gives an error: The weights trying to be saved contained shared tensors 
            # [{'model.embeddings.position_ids', 'bert.embeddings.position_ids'}] that are mismatching 
            # the transformers base configuration. Try saving using `safe_serialization=False` or remove this tensor sharing.
            #model.save_pretrained(dir_path)
            state_dict = model.state_dict()
            state_dict = {k:v for k, v in state_dict.items() if "embeddings.position_ids" not in k}
            model.save_pretrained(dir_path, state_dict=state_dict, safe_serialization=True)
