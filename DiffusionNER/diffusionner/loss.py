...
class DiffusionNERLoss(Loss):
    ...
    def compute(self, output, gt_types, gt_spans, entity_masks, epoch, batch = None):
        ...
        pred_logits, x, y, pred_left, pred_right = output["pred_logits"], output["pred_spans"][:, :, 0], output["pred_spans"][:, :, 1], output["pred_left"], output["pred_right"]

        if torch.isnan(pred_logits).any():
            print('harl')
        if torch.isnan(pred_left).any():
            print('harl')
        if torch.isnan(pred_left).any():
            print('harl')
...
class Criterion(nn.Module):
    ...
    def loss_boundary(self, outputs, targets, indices, num_spans):
        ...
        # NIL object boundary
        if torch.isnan(left_nll_loss).any() or torch.isnan(right_nll_loss).any() or torch.isnan(token_masks).any():
            print("harl")
