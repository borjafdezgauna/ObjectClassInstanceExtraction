...
class PIQNTrainer(BaseTrainer):
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset,
        ...
        data_loader = DataLoader(dataset, batch_size=args.train_batch_size, shuffle=shuffle, drop_last=True,
                                    ...
                                    num_workers=0,
                                    collate_fn=sampling.collate_fn_padding,  sampler=train_sampler)
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            #print(f"Encodings:\n{batch['encodings']}\n\n")
            #print(f"Context masks:\n{batch['context_masks']}\n\n")
            #print(f"Seg encoding:\n{batch['seg_encoding']}\n\n")
            #print(f"context2token_masks:\n{batch['context2token_masks']}\n\n")
            #print(f"token_masks:\n{batch['token_masks']}\n\n")
            #print(f"pos_encoding:\n{batch['pos_encoding']}\n\n")
            #print(f"wordvec_encoding:\n{batch['wordvec_encoding']}\n\n")
            #print(f"char_encoding:\n{batch['char_encoding']}\n\n")
            #print(f"token_masks_char:\n{batch['token_masks_char']}\n\n")
            #print(f"char_count:\n{batch['char_count']}\n\n")
