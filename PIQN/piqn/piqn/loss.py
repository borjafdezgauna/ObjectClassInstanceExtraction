...
class PIQNLoss(Loss):
    ...
    def compute(self, entity_logits, pred_left, pred_right, output, gt_types, gt_spans, entity_masks, epoch, deeply_weight = "same", seq_logits = None, gt_seq_labels = None, batch = None):
        ...
        for out_dict in output:
            ...
            a = torch.isnan(entity_logits).any()
            b = torch.isnan(pred_left).any()
            c = torch.isnan(pred_right).any()
            if a or b or c:
                print("harl")
...
class Criterion(nn.Module):
    ...
    def loss_boundary(self, outputs, targets, indices, num_spans):
        ...
        if torch.isnan(src_spans_left).any():
            print("harl")
        src_spans_right = outputs['pred_right'][idx]
        if torch.isnan(src_spans_right).any():
            print("harl")
        ...
        if torch.isnan(left_onehot).any():
            print("harl")
        ...
        if torch.isnan(right_onehot).any():
            print("harl")
        ...
        losses = {}
        if torch.sum(num_spans).item() == 0:
            print("0 spans -> loss = 0")
            losses['loss_boundary'] = torch.tensor(0, device = loss_boundary.device)
            return losses
