...
class Evaluator:
    ...
    def eval_batch(self, entity_logits, p_left, p_right, outputs, batch = None):
        ...
        if multihead:
            ...
            for out_dict in outputs:
                ...
                entity_prob = F.log_softmax(entity_logits, -1) # entity_logits.softmax(-1)
                entity_prob = torch.exp(entity_prob)
    ...
    def _convert_pred_entities(self, pred_types, pred_spans, pred_scores,  left_scores, right_scores, doc):
        converted_preds = []
